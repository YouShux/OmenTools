...
namespace OmenTools.Threading.TaskHelper;
...
public partial class TaskHelper
{
    ...
    public void Enqueue(TaskHelperTask task, int weight = 0)
    {
        ...
        if (!TaskChannel.Writer.TryWrite(new AddTaskMessage(task, weight))) return;
    ...
    {
        if (!TaskChannel.Writer.TryWrite
                (new AddTaskMessage(new(task, name, timeoutMS, timeoutBehaviour, exceptionBehaviour, timeoutAction, exceptionAction), weight))) return;
        Interlocked.Increment(ref pendingTaskCount);
    }
    ...
    {
        if (!TaskChannel.Writer.TryWrite
                (new AddTaskMessage(new(asyncTask, name, timeoutMS, timeoutBehaviour, exceptionBehaviour, timeoutAction, exceptionAction), weight))) return;
        Interlocked.Increment(ref pendingTaskCount);
    }
